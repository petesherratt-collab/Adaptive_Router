...
BOOTSTRAP_SEED = "20260906-runtime-v0.3-shadow-json-v1"
...
def _bootstrap(rows, task_ids):
    ...
    for draw in range(BOOTSTRAP_DRAWS):
        ...
        if not sampled:
            undefined += 1
            continue
        remote_correct = sum(row["remote_oracle"]["correct"] for row in sampled)
        selected = [_counterfactual(row) for row in sampled]
        counterfactual_correct = sum(item[1] for item in selected)
        differences.append(
            (counterfactual_correct - remote_correct) / len(sampled)
        )
    ...
    return {
        ...
        "seed": BOOTSTRAP_SEED,
        "counterfactual_minus_remote_rate_ci95": [
...
def analyze_rows(rows, tasks, revision):
    ...
    accepted_upper = clopper_pearson_upper(
    ...
    conditions = {
        ...
        "counterfactual_correctness_at_least_remote": (
            overall["counterfactual_correct_count"] >= overall["remote_correct_count"]
        ),
        "at_least_60_remote_calls_avoided": overall["remote_calls_avoided"] >= 60,
        "counterfactual_median_at_most_remote": (
    ...
    report = {
        "schema_version": "runtime_v0_3_shadow_json_analysis_v1",
...
def authenticate_complete(root=pv.ROOT, frozen_root=None):
    ...
    pv.validate_rows(rows, tasks, revision)
    if (
        summary.get("schema_version") != "runtime_v0_3_shadow_json_summary_v1"
