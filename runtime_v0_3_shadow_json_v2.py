"""Frozen schema, oracle, budget, and evidence rules for runtime v0.3 shadow JSON V2."""
...
SCHEMA_VERSION = "runtime_v0_3_shadow_json_observation_v2"
...
def validate_benchmark(document):
    ...
    if (
        document["schema_version"] != "runtime_v0_3_shadow_json_benchmark_v2"
...
def output_paths(root=ROOT):
    root = Path(root)
    prefix = "runtime_v0_3_shadow_json_v2_"
    return {
...
def summary(rows, revision, budget):
    return {
        "schema_version": "runtime_v0_3_shadow_json_summary_v2",
        "suite_id": SUITE_ID,
