...
import analyze_runtime_v0_3_shadow_json_v2 as analyzer
import run_runtime_v0_3_shadow_json_v2 as runner
...
class FrozenDesignTests(unittest.TestCase):
    ...
    def test_json_oracle_is_recursive_and_type_sensitive(self):
        ...
        task = inventory["v2_nested_10"]
        exact = '{"bundle":"B-11","values":["beta",5,null],"flags":{"priority":true,"audited":false}}'
        wrong_order = '{"bundle":"B-11","values":[5,"beta",null],"flags":{"priority":true,"audited":false}}'
        wrong_type = '{"bundle":"B-11","values":["beta","5",null],"flags":{"priority":true,"audited":false}}'
    ...
    def test_shape_can_pass_while_oracle_fails(self):
        ...
        task = inventory["v2_flat_01"]
        raw = '{"lot":"wrong","custodian":"wrong","pieces":0,"sealed":false}'
        self.assertEqual(runner._contract_result(task, raw)["status"], "PASS")
        self.assertFalse(pv.oracle(task, raw).correct)

    def test_v2_tasks_do_not_reuse_v1_content(self):
        v1 = json.loads(
            (ROOT / "benchmark_runtime_v0_3_shadow_json_v1.json")
            .read_text(encoding="utf-8")
        )
        v2, _, _ = pv.load_frozen_inputs(ROOT)
        for selector in (
            ...
            self.assertFalse(
                {selector(task) for task in v1["tasks"]}
                & {selector(task) for task in v2["tasks"]}
            )
...
class ProviderOutcomeTests(unittest.TestCase):
    def execute_synthetic(self, directory, remote_failure=None, local_failure=None):
        ...
        report = runner.execute_observations(
            tasks,
            config,
            "synthetic-v2-provider-outcomes",
            pv.MODEL_SPEC,
            Path(directory),
            local_fake,
            remote_fake,
        ...
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaisesRegex(RuntimeError, "synthetic transport escape"):
                runner.execute_observations(
                    tasks,
                    config,
                    "synthetic-v2-instrument-failure",
                    pv.MODEL_SPEC,
                    Path(directory),
            ...
            self.assertEqual(
                ...
                "runtime_v0_3_shadow_json_failure_v2",
            )
            ...
            self.assertEqual(manifest["task_id"], "v2_flat_01")
